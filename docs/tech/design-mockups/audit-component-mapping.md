...
| Segmented filter chips (`.chips`/`.chip`, `is-active`) | admin animals toolbar ("All / Dogs / Cats / Small"), media filter | `ToggleGroup` | — | New finding: this is a pill-track segmented control, closest match is shadcn's `ToggleGroup` (Radix Toggle Group) — **not currently installed or in the "to install" list**, worth adding alongside #137's items |
